package com.Elevator.ValueObjects;

import java.util.*;
import com.Elevator.Simulators.*;
...
public class Floor {
    ...
    public Floor(int floorNumber) {
        ...
        if (Simulator.debug) {
            log = new Logger("Floor" + this.floorNumber);
        }
    }
    ...
    public void summonElevatorUp(Person person) {
        if (Simulator.debug) {
            log.write("Summon up for person " + person.getPersonNumber());
        }
        upWaiting.add(person);
        if (!summonUp) {//if already summoned no need to do it again
            if (Simulator.debug) {
                log.write("Light off summon UP for person " + person.getPersonNumber());
            }
    ...
    public void summonElevatorDown(Person person) {
        if (Simulator.debug) {
            log.write("Summon down for person " + person.getPersonNumber());
        }
        downWaiting.add(person);
        if (!summonDown) { // id already summoned no need to do it again
            if (Simulator.debug) {
                log.write("Light off summon DOWN for person " + person.getPersonNumber());
            }
    ...
    public void elevatorArrivedUp(Elevator elevator) {
        ...
        if (Simulator.debug) {
            log.write("Elevator " + elevator.getElevatorNumber() + " has arrived UP on " + getFloorNumber());
        }
    } // end elevatorArrived
    ...
    public void elevatorArrivedDown(Elevator elevator) {
        ...
        if (Simulator.debug) {
            log.write("Elevator " + elevator.getElevatorNumber() + " has arrived DOWN on " + getFloorNumber());
        }
    }

    public void stopWaiting(Person person) {
        if (Simulator.debug) {
            log.write("Person " + person.getPersonNumber() + "  has stopped waiting on " + getFloorNumber());
        }
